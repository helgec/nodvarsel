...
# --- KONFIGURASJON ---
# Gå inn på https://www.nodvarsel.no/rss/ , høyreklikk på "RSS - aktive Nødvarsler" 
# og velg "Kopier lenkeadresse". Lim den inn under:
RSS_URL = "https://www.nodvarsel.no/api/rss/aktive" # Bytt ut om den eksakte URL-en avviker

# Slack Webhook - sett denne som miljøvariabel eller hardkod den her
SLACK_WEBHOOK = os.environ.get("SLACK_WEBHOOK_URL", "DIN_WEBHOOK_URL_HER")

# Filsti for å huske hvilke varsler som allerede er pushet til desken (unngå duplikater)
BASE_DIR = os.path.dirname(os.path.abspath(__file__))
SEEN_FILE = os.path.join(BASE_DIR, "seen_nodvarsler.json")

# Sett opp logging (Veldig kjekt når scriptet snurrer i bakgrunnen via cron)
...
if not SLACK_WEBHOOK_URL:
    raise ValueError("Kritisk feil: SLACK_WEBHOOK_URL er ikke konfigurert i .env")
...
def save_seen(seen_list):
    """Lagrer ID-er for sendte varsler. Tar vare på de siste 50 for å holde filen liten."""
...
def send_to_slack(entry):
    ...
    # Setter sammen en Slack payload med Blocks som skriker litt oppmerksomhet
    payload = {
        "text": f"🚨 NYTT NØDVARSEL: {title}", # Fallback for pushvarsler
        "blocks": [
            {
                "type": "header",
                "text": {
                ...
                "elements": [
                    {
                        ...
                        "url": link,
                        "style": "danger" # Gir knappen en rød varsel-farge i Slack
                    }
                ]
            }
        ]
...
def main():
    if SLACK_WEBHOOK == "DIN_WEBHOOK_URL_HER":
        logging.error("Du må legge inn en gyldig SLACK_WEBHOOK_URL før scriptet kan kjøres.")
        sys.exit(1)
    ...
    # Går gjennom varslene fra RSS-feeden
    for entry in feed.entries:
        # Henter ut unik ID, med lenken som fallback
